...
export const getEmployedPeople = (url) => {
    return fetch(url)
        .then(res => res.json())
        .then((people) => people.filter((people) => people.isEmployed))
};
...
export const findPersonWithId = (url, id) => {
    return fetch(url)
        .then((res) => res.json())
        .then((people) => {
            const SearchPerson = people.find((person) => person.id === id);
            return SearchPerson ? SearchPerson : "Person not found";
        });
};
...
export const getPeopleWithMatchingInterests = (url, interest) => {
    return fetch(url)
        .then((res) => res.json())
        .then((people) => {
            const interestMatched = people.filter((person) => {
                return person.interests.includes(interest);
            });

            return interestMatched.length ? interestMatched : "No people with interest found";
        });
};
    ...
    return fetch(peopleUrl)
        ...
        .then((people) => {
            return fetch(interestsUrl)
                .then((res) => res.json())
                .then((interests) => {
                    const peopleInformation = people.map((person) => {
                        const newPerson = {...person };
