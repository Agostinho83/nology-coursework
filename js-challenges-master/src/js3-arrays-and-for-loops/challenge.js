...
export const getFirstAndLastItems = (itemsArr) => {
    const newArray1 = itemsArr[0];

    const newArray2 = itemsArr[itemsArr.length - 1];
...
export const moveFirstAndLastItems = (itemsArr) => {
    ...
    const moveLastItems = newArr.pop();
    newArr.unshift(moveLastItems);
    return newArr;
...
export const reverseOrder = (toReverseArr) => {
    ...
    const rvr = [].concat(toReverseArr).reverse();
    return rvr;
};
...
export const generateHighscores = (playersArr, scoresArr) => {
    ...
    while (scores.length !== playersArr.length) {
        ...
        const msg = `P:${i + 1} ${playersArr[i]} scored ${scoresArr[i]}`;
        scores.push(msg);
    }
...
export const encryptString = (toEncrypt) => {
    ...
    for (let index = 0; index < toEncrypt.length; index++) {
        pivot[index % 3].push(toEncrypt.charAt(index));
    }
    let String = "";
    for (let index = 0; index < pivot.length; index++) {
        for (let join = 0; join < pivot[index].length; join++) {
            String += pivot[index][join];
        }
    }
    return String;
};