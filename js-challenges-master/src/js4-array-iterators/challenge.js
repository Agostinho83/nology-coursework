...
export const createPercentageList = (numbersArr) => {
    const mapPercent = numbersArr.map((number) => (number * 100) + "%");
    return mapPercent;
...
export const createListOfPoessessions = (possessionsArr, name) => {
    const arrList = possessionsArr.map((possession) => `${name} ${possession}`);
    return arrList;
...
export const convertStringToNumbersArray = (numberString) => {
    const arrayStr = numberString.split("+").map((number) => Number(number));
    return arrayStr;
...
export const createOddEvenArray = (numberString) => {
    const arrayNum = convertStringToNumbersArray(numberString);
    const oddOrEven = arrayNum.map((num) => (num % 2 === 0 ? "even" : "odd"));
    return oddOrEven;
};
...
export const filterBooksBySearch = (booksArr, searchTerm) => {
    const filteredSearch = booksArr.filter((book) => book.includes(searchTerm));
...
export const formatString = (string) => {
    ...
    //The \w looks for all word characters in a string(character from a-z, A-Z, 0-9, including the _ (underscore) character)
    // The \W uppercase is not word
...
export const fizzBuzz = (mixedArray) => {
    ...
    const arrayPositNum = mixedArray.filter((item) => item > 0 && Number(item));

    const fizzBuzz = arrayPositNum.map((num) => {
        let string = "";

        if (!(num % 3)) string = string + "Fizz";
        if (!(num % 5)) string = string + "Buzz";
